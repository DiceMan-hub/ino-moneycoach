...
def get_nights_cost(hotel, start_date_str, nights, offer_type=None):
    ...
    if offer_type == '3rd_Free' and nights >= 3:
        # Subtract the 3rd night cost (average or last night)
        # Let's subtract the last night cost for simplicity
        if len(details) >= 3:
            deduct = details[2]
            cost -= deduct
...
# Period: Aug 23 - Sep 22 (30 nights)
# Checkout Sep 22.
plan = [
    # BKK
    ...
    # Last Night (Back to BKK or stay somewhere?)
    # Sep 21-22. FHR 1 night.
    # Kimpton Stay 2.
...
for p in plan:
    ...
    # Format date range: 8/23〜8/26 (checkout is 8/26)
